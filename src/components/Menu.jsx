...
const Menu = () => {
  ...
  return (
    <main className={`grid justify-end items-start fixed ${isShowMenu ? "right-0":"-right-[200%]"} transition-all duration-700 w-full top-0 h-screen z-20`}>
        <div onClick={handleClickChangeClose} className={`${isShowMenu ? "right-0" : "-right-[200%]"} transition-all duration-500 fixed top-0 w-full h-full bg-gray-500/20`}></div>
        <div className={`w-[350px] h-full bg-white relative rounded-md `}>
            ...
            <div className='text-center pt-6 w-[80px] h-[80px] mx-auto'>
                <img src="/img/Toñex.png" alt="" className='rounded-full '/>
            </div>
            <div className='grid items-center justify-center h-full py-10 text-center text-xl font-bold'>
                <ScrollLink onClick={handleClickChangeClose} className='border-b-[1px] border-dark cursor-pointer font-semibold hover:bg-blue-500/50 px-5 py-4 rounded-md' to="home" smooth={true} duration={800}>Home</ScrollLink>
